...
def player(board):
    ...
    for row in board:
        playerX_count += row.count(X)
        playerO_count += row.count(O)
        empty_count += row.count(EMPTY)
    # If X has more squares than O, its O's turn:
    return O if playerX_count > playerO_count else X
...
def actions(board):
    ...
    """
    return set(((i,j) for j in range(3)  for i in range(3) if board[i][j] == EMPTY ))
...
def result(board, action):
    ...
    if i not in [0, 1, 2] or j not in [0, 1, 2]:
      raise Exception(action, board, 'Result function given an invalid board position for action: ')
    elif board[i][j] != EMPTY:
      raise Exception(action, board, 'Result function tried to perform invalid action on occupaied tile: ')
...
def winner(board):
    ...
    # Check Diagonals:
    diag1 = str(board[0][0]) + str(board[1][1])+str(board[2][2])
    diag2 =  str(board[0][2])+ str(board[1][1])+str(board[2][0]) 
    return X if diag1 == 'XXX' or diag2 == 'XXX' else O if diag2 == 'OOO' or diag1 == 'OOO'  else None
